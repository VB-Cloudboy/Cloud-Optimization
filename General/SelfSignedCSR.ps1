...
.DESCRIPTION
Tested platforms:
- Windows 10 with PowerShell 5.1.18
Created By:
Vijay Borkar
...
# Output the CSR
$CSR = Get-Content $files['csr'] | Out-File -FilePath "$DesktopPath\mycertcsr.req"
Get-Content $files['csr'] | Out-File -FilePath "$DesktopPath\mycertcsr.req"
Write-Output $CSR
Write-Host "
"
